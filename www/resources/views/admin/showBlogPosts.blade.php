...
@section('main')
	...
	<div class="row">
		<div class="col-md-3">
			<a href="/master/blog/posts/add" class="thumbnail text-center">
		...
		<div class="col-md-12">
			@if( count($posts) > 0 )
				{!! Form::open(['class' => 'posts']) !!}
								...
								<tr>
									...
									<td class="text-right">
										<div class="btn-group" role="group">
											<button type="button" class="btn {{ !empty($post->visible) ? 'btn-success' : 'btn-default' }} visible" title="Показать/Скрыть запись" data-id="{{ $post->id }}">
												<i class="fa {{ !empty($post->visible) ? 'fa-star' : 'fa-star-o' }}"></i>
											</button>
											<a href="/master/blog/posts/edit/{{ $post->id }}" class="btn btn-warning" title="Редактировать запись">
												<i class="fa fa-pencil"></i>
											</a>
											<a href="/blog/{{ $post->category_slug }}/{{ $post->slug }}" class="btn btn-primary" title="Открыть в новом окне" target="_blank">
												<i class="fa fa-share"></i>
											</a>
											<button type="button" class="btn btn-danger delete" title="Удалить запись" data-id="{{ $post->id }}">
					...
					{!! $posts->appends(['category' => (int)Request::get('category')])->render() !!}
	...
	<script>
		$(function(){
			...
			$('.visible').on('click', function(event){
				...
				$.post('{{ URL::to("master/blog/posts/visible") }}', data, function(data){ console.log(data.message) }, 'JSON');
			});
		});
	</script>

...
@section('main')
	...
	<div class="row">
		<div class="col-md-3">
			<a href="/master/users/add" class="thumbnail text-center">
	...
	<div class="row">
		<div class="col-md-12">
			@if( count($usersList) > 0 )
					...
					<table class="table">
						...
						<tbody>
							@foreach($usersList as $user)
								<tr>
									<td>
										<input type="checkbox" name="check[]" value="{{ $user->id }}" @if( Auth::user()->id == $user->id ) disabled @endif />
										&ensp;{{ $user->name }}
									</td>
									<td class="text-right">
										<div class="btn-group" role="group">
											<a href="/master/users/edit/{{ $user->id }}" class="btn btn-warning" title="Редактировать данные о пользователе"><i class="fa fa-pencil"></i></a>
											<button type="button" class="btn btn-danger delete" title="Удалить пользователя" data-id="{{ $user->id }}" @if( Auth::user()->id == $user->id ) disabled @endif><i class="fa fa-times"></i></button>
										</div>
									</td>
								</tr>
							@endforeach
				...
				{!! Form::close() !!}
			@else
				<div class="alert alert-warning">Пользователи еще не созданы</div>
			@endif
		</div>
	</div>
	<script>
